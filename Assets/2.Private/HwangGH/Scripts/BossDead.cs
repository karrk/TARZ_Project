...
public class BossDead : Action
{
    ...
    {
        selfObject = GameObject.FindGameObjectWithTag("Monster");
        // 애니메이터 컴포넌트 가져오기
    ...
    public override TaskStatus OnUpdate()
    {
        ...
        if (health.Value <= 0)
        {
            if (selfObject != null && selfObject.Value != null)
            {
                ...
                if (animator != null)
                {
                    animator.SetTrigger("Death"); // Die 애니메이션을 트리거로 설정
                }
    ...
    private bool IsDieAnimationFinished()
    {
        if (animator != null)
        {
            ...
            return stateInfo.IsName("Arnold_Die") && stateInfo.normalizedTime >= 0.7f; // normalizedTime이 1 이상이면 애니메이션 종료
        }
        return false;
    }
}